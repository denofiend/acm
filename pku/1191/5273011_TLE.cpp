...
double f[MAXN*2][MAXN][MAXN][MAXN][MAXN];
double sum[MAXN][MAXN];
int n;

double get(int x0, int y0, int x1, int y1){
	double ret = sum[x1][y1];
...
double rec(int n, int i0, int j0, int i1, int j1){
	...
	FOR (x, i0, i1)
	{
		double xx = get(i0, j0, x, j1) * get(i0, j0, x, j1);
		double xxx = rec(n-1, x+1, j0, i1, j1);
	...
	FOR (y, j0, j1)
	{
		double yy = get(i0, j0, i1, y) * get(i0, j0, i1, y);
		double yyy = rec(n-1, i0, y+1, i1, j1);
